...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Errors handled inside context via toast
    } finally {
      setLoading(false);
    }
  };
    ...
    <main className="min-h-screen bg-[#FAF8F5] text-[#1A1A1A] flex flex-col font-sans">
      ...
      <div className="flex-1 flex items-center justify-center py-12 px-4">
        <div className="bg-white border border-[#E8E3DA] rounded-2xl shadow-lg p-8 sm:p-12 w-full max-w-[460px] text-center">
          <div className="mb-6">
            <h1 className="font-serif text-3xl font-medium tracking-widest text-[#1A1A1A] mb-3">
              SIGN IN
            </h1>
            <p className="text-xs text-gray-500 leading-relaxed max-w-[340px] mx-auto">
              Welcome back to Jewellery Garden. Enter your credentials to access your account.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            <div className="flex flex-col gap-2 text-left">
              <label className="text-[11px] font-semibold tracking-wider text-[#1A1A1A] uppercase">
                EMAIL ADDRESS
              </label>
            ...
            <div className="flex flex-col gap-2 text-left">
              ...
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full pl-10 pr-11 py-3 border border-[#E8E3DA] rounded-lg bg-white text-sm text-[#1A1A1A] focus:border-[#C8232A] focus:ring-1 focus:ring-[#C8232A] focus:outline-none transition-all"
                  required
                />
                <button
                  type="button"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 bg-[#C8232A] hover:bg-[#A81B21] text-white font-semibold text-xs tracking-widest rounded-lg transition-all shadow-md flex items-center justify-center mt-2 cursor-pointer"
            >
          ...
          <Link
            href="/signup"
            className="block w-full py-3.5 border border-[#C8232A] text-[#C8232A] hover:bg-red-50 font-semibold text-xs tracking-widest rounded-lg transition-all text-center"
          >
            CREATE YOUR ACCOUNT
          </Link>
        </div>
